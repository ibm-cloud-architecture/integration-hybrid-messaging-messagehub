...
var receiveClient = mqlight.createClient({service: my_service});
...
receiveClient.on('message', function(data, delivery) {
  console.log('<<< Received from MQ: %s, topic: %s, ttl: %d', data
                                                        , delivery.message.topic
                                                        , delivery.message.ttl);
  sendClient.send(out_topic, data);
  console.log('>>> Sent to Message Hub: %s, topic: %s, ttl: %d', data
                                                        , delivery.message.topic
                                                        , delivery.message.ttl);
});
